# TWEETBAR - Complete Project Documentation
## A Modern Twitter-like Social Platform Built with Django
...
7. [Forms (Input Validation)](#forms-input-validation)
8. [Views (Business Logic)](#views-business-logic)
9. [Templates (UI Layer)](#templates-ui-layer)
...
13. [Cloud Configuration](#cloud-configuration)
14. [Admin Dashboard & User Management](#admin-panel--user-management-new)

---

## PROJECT OVERVIEW

**Project Name:** Hunain's Tweetbar v2.0  
**Technology:** Django 6.0  
**Type:** Social Media Platform / Micro-blogging Application  
**Database:** PostgreSQL (Supabase) / SQLite (Local Development)  
**Deployment:** Vercel + Supabase  
**Purpose:** A modern, real-time platform where users can share tweets, like content, comment on tweets, and view other users' profiles
...
- Image uploads with tweets (powered by Supabase Storage)
...
### Directory Tree:
```
hunain_project/                 # Django project root
├── hunain_project/             # Main project configuration
│   ├── settings.py             # Django configuration & environment setup
│   ├── urls.py                 # Main URL routing (entry point)
│   ├── wsgi.py                 # WSGI application for production
│   └── asgi.py                 # ASGI application for async support
│
├── tweet/                       # Main Django app (business logic)
│   ├── models.py               # Database models (Tweet, Comment)
...
│
├── certs/                      # HTTPS certificates for local dev
│   ├── 127.0.0.1+localhost.pem
│   └── 127.0.0.1+localhost-key.pem
│
...
├── vercel.yaml                 # Vercel deployment config
├── render.yaml                 # Render platform config
├── Procfile                    # Heroku/Production config
...
### Overview:
The application uses a relational database with 3 main tables (plus Django's built-in auth tables):
...
└────────────────────────────────┘        │
         │                                │
         └────────────────────────────────┘
        Related back to User
```

### Table 1: User (Django Built-in)
**Purpose:** Stores user credentials and authentication data  
**Source:** Django's `django.contrib.auth.models.User`
...
| is_staff | BOOLEAN | DEFAULT FALSE | Admin staff status |
| is_superuser | BOOLEAN | DEFAULT FALSE | Superuser/admin status |
| date_joined | TIMESTAMP | AUTO | Account creation datetime |
| last_login | TIMESTAMP | NULL | Last login datetime |

### Table 2: tweet_tweet (Custom Model)
**Purpose:** Stores user tweets/posts with optional images

**Creation Migration:** `0001_initial.py` (2026-03-28)

| Column | Type | Constraint | Purpose |
|--------|------|-----------|---------|
| id | BIGINT | PRIMARY KEY, AUTO_INCREMENT | Unique tweet identifier |
| user_id | BIGINT | FOREIGN KEY → auth_user.id, CASCADE | Tweet owner |
| text | TEXT | VARCHAR(240) | Tweet content (limited to 240 chars) |
| photo_url | URL | NULL, BLANK | Public URL of image stored in Supabase |
| created_at | TIMESTAMP | AUTO_NOW_ADD | Creation timestamp |
| updated_at | TIMESTAMP | AUTO_NOW | Last update timestamp |

**Additional Field:** 
- `likes` - ManyToMany relationship to User (added in migration 0002)

| Column | Type | Constraint | Purpose |
|--------|------|-----------|---------|
| likes (M2M through table) | - | - | Users who liked this tweet |

**M2M Junction Table Created:** `tweet_tweet_likes`
```
┌────────────────────────────┐
│   tweet_tweet_likes        │
│ ───────────────────────── │
...
### Table 3: tweet_comment (Custom Model)
**Purpose:** Stores comments/replies on tweets

**Creation Migration:** `0002_tweet_likes_comment.py` (2026-03-31)

| Column | Type | Constraint | Purpose |
|--------|------|-----------|---------|
| id | BIGINT | PRIMARY KEY, AUTO_INCREMENT | Unique comment identifier |
| tweet_id | BIGINT | FOREIGN KEY → tweet_tweet.id, CASCADE | Parent tweet |
| user_id | BIGINT | FOREIGN KEY → auth_user.id, CASCADE | Comment author |
| text | TEXT | VARCHAR(240) | Comment content |
| created_at | TIMESTAMP | AUTO_NOW_ADD | Creation timestamp |
...
1. **User → Tweet (One-to-Many)**
   ...
   - When user is deleted, all their tweets are deleted (CASCADE)

2. **User → Tweet (Many-to-Many via likes)**
   - One user can like many tweets
   - One tweet can be liked by many users
   - Records stored in `tweet_tweet_likes` bridge table

3. **User → Comment (One-to-Many)**
   - One user can have many comments
   - When user is deleted, all their comments are deleted (CASCADE)

4. **Tweet → Comment (One-to-Many)**
   - One tweet can have many comments
   - When tweet is deleted, all its comments are deleted (CASCADE)

---

## SETTINGS & CONFIGURATION

### File Location: `hunain_project/settings.py`

This is the core Django configuration file that controls how your application behaves.

#### 1. **Environment Detection**
```python
# Detects if running on Vercel hosting
IS_VERCEL = any(x in os.environ for x in ["VERCEL", "VERCEL_ENV", "VERCEL_URL"])
```
**Purpose:** Differentiates between local development and production deployment
...
# Vercel production: DEBUG=False (more secure)

ALLOWED_HOSTS = ['localhost', '127.0.0.1']
# On Vercel: dynamically adds .vercel.app and specific URLs
```
