...
class TweetbarTestCase(TestCase):
    ...
    def test_tweet_creation_with_image(self, mock_upload):
        ...
        self.client.post(
            reverse("tweet_create"), {"text": "Tweet with image", "photo": image}
        )
    ...
    def test_tweet_creation_without_image(self):
        self.client.login(username="testuser", password="password123")
        self.client.post(reverse("tweet_create"), {"text": "Tweet without image"})
    ...
    def test_comment_creation(self):
        self.client.login(username="testuser", password="password123")
        self.client.post(
            reverse("add_comment", args=[self.tweet.id]), {"text": "Nice tweet!"}
        )
        self.assertEqual(
            Comment.objects.filter(tweet=self.tweet, text="Nice tweet!").count(), 1
        )
    ...
    def test_authorization_edit_tweet(self):
        ...
        response = self.client.post(
            reverse("tweet_edit", args=[self.tweet.id]), {"text": "Hacked text"}
        )
